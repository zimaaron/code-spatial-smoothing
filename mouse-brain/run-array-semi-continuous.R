...
library(ggplot2)
library(INLA)
dir.create(file.path("/home/aeoz001/scratch", j.i))
INLA:::inla.dynload.workaround("Rocky Linux-8", path = file.path("/home/aeoz001/scratch", j.i))
...
write.csv(rbind(nospatial=tapply(out.nospatial$dic$local.dic, out.nospatial$dic$family, sum),
          ...
          file.path(o.d, glue("{l.n}-DIC-comp.csv")))
...
# as expected
c(yPositive.obs = mean(lr.d[which(lr.d > 0)]),
...
png(file.path(o.d, glue("{l.n}-post-marg-dist.png")), width = 8, height = 8, units = "in")
...
png(file.path(o.d, glue("{l.n}-spatial-effects-ordered.png")), width = 8, height = 8, units = "in")
...
png(file.path(o.d, glue("{l.n}-spatial-effects-marginals.png")), width = 8, height = 8, units = "in")
par(mfrow=c(2,2))
...
xy.pred <- make.pred.grid(xy.obs = xy.obs,
                          ...
                          res = pred.grid.res)
if(j.i == 1){
  write.csv(xy.pred, file.path(o.d, "xy-pred-locs.csv"), row.names = F)
}
...
png(file.path(o.d, glue("{l.n}-spatial-fields.png")),
...
png(file.path(o.d, glue("{l.n}-fgf1-response-occurrence-fields.png")),
    width = 12, height = 8, units = "in", res = 500)
